...
**修改为：**
```go
module github.com/zhucl121/langchain-go  // ✅ 可以被 go get
...
```bash
# macOS
find . -name "*.go" -type f -exec sed -i '' 's|"github.com/zhucl121/langchain-go/|"github.com/zhucl121/langchain-go/|g' {} +
...
Get-ChildItem -Recurse -Filter *.go | ForEach-Object {
    (Get-Content $_.FullName) -replace 'langchain-go/', 'github.com/zhucl121/langchain-go/' | 
...
[![License](https://img.shields.io/github/license/zhucl121/langchain-go)](LICENSE)
...
REPO_PATH="github.com/zhucl121/langchain-go"

echo "Replacing import paths..."

# 替换 Go 文件
find . -name "*.go" -type f -not -path "*/vendor/*" -exec sed -i.bak \
  -e "s|\"github.com/zhucl121/langchain-go/|\"${REPO_PATH}/|g" {} \;
