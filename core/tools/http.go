...
// HTTPRequestToolConfig 是 HTTPRequestTool 配置。
...
// 示例：
//
//	tool := tools.NewHTTPRequestTool(nil)
//	result, _ := tool.Execute(ctx, map[string]any{
//	    "method": "PUT",
...
//	})
//
func NewHTTPRequestTool(config *HTTPRequestToolConfig) *HTTPRequestTool {
	var client *http.Client
	...
	return &HTTPRequestTool{
		name: "http_request",
		description: `Send an HTTP request with any method (GET, POST, PUT, DELETE, etc.) and return the response.
Parameters:
- method: HTTP method (GET, POST, PUT, DELETE, PATCH, etc.) (required)
- url: The URL to send the request to (required)
- body: The request body (optional, for POST/PUT/PATCH)
- headers: HTTP headers as a map (optional)`,
		client: client,
	}
}
...
func (t *HTTPRequestTool) Execute(ctx context.Context, args map[string]any) (any, error) {
	...
	if !ok {
		return nil, fmt.Errorf("%w: 'method' must be a string", ErrInvalidArguments)
	}
